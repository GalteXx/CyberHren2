...
void    cir(int x_c, int y_c, char sim_c, vector<vector<char> > &arr, int rad, int w, int h, char backk)
{
    int kol = 1;
    bool fl = 1;
    ...
    for(int i = 0; i < h; i++){
            for(int j = 1; j < w; j++)
            {
                int j1 = j;
                if(arr[i][j] == sim_c)
                while(arr[i][j1] == sim_c)
                {
                    kol++;
                    j1++;
                }
                if(arr[i][j-1] == sim_c && arr[i][j] == backk)
                {
                    if(kol <= 4)
                    {
                        while(arr[i][j] == backk && fl == 1)
                        {
                            arr[i][j] = sim_c;
                            j++;
                        }
...
int     main()
{
    ...
    {
        cin >> x_c >> y_c >> sim_c >> rad;
        cir(x_c, y_c, sim_c, arr, rad, w, h, backk);
    }
    if(act == 'L')
    {
        int x1, y1, x2, y2;
